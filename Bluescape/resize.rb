...
browser.find_element(:class, "img-container").click  # Click on image to display controls

delete_button = browser.find_element(:class, "window-delete-button")
resize_button = browser.find_element(:class, "window-resize-button")
pin_icon = browser.find_element(:class, "pin-icon")
location = resize_button.location.to_a
puts "Image location is: #{location}"

# Click and hold "resize" button
browser.action.click_and_hold(resize_button).perform
sleep(1)

# Move by 50 pixels and release
browser.action.move_by(50, 50).release.perform
new_location = resize_button.location.to_a
puts "New location is: #{new_location}"